---
title: 智慧100V6.2部署指南（虚拟机版）
date: 2023-05-26T17:02:05
---

# 智慧100V6.2完整部署指南（虚拟机版）

* [智慧100V6.2完整部署指南（虚拟机版）](#智慧100v62完整部署指南虚拟机版)

  * [前置声明](#前置声明)
  * [一、系统环境准备](#一系统环境准备)
  * [1\. 申请APP企业名称](#1-申请app企业名称)
  * [2\. 申请安全组放通端口](#2-申请安全组放通端口)
  * [3\. 确认svn账号权限是否已申请](#3-确认svn账号权限是否已申请)
  * [4\. 整理服务部署清单](#4-整理服务部署清单)
  * [5\. 磁盘分区和挂载](#5-磁盘分区和挂载)
  * [6\. 创建交换分区](#6-创建交换分区)
  * [7\. 安装内部下载工具](#7-安装内部下载工具)
  * [二、安装基础服务](#二安装基础服务)
  * [1\. 安装JDK](#1-安装jdk)
  ...
  * [9\. 安装 kkFileView](#9-安装-kkfileview)
  ...
  * [单个业务服务安装](#单个业务服务安装)
    ...
    * [dynamic-bizengine\_plugins](#dynamic-bizengine_plugins)
    * [注意事项](#注意事项-3)
    * [impexp](#impexp)
...
A: 安装包下载位置可以通过服务部署清单配置文件 `deployment.yaml` 中的 `deploy_home.package` 选项进行调整。目前归档的配置，此选项设置为 `/home/apaas/service_deployment_package`，因为在多数实施环境，数据盘挂载在 `/home/apaas` 目录，磁盘空间不会太紧张。若服务器无法直接访问 svn，可以在其他机器上执行 `pull` 命令进行下载，下载完成后，再通过 `scp` 命令或其他方式将此目录同步到服务器上。
